...
import getAssetValues, {
    getProfit,
    getCommission,
} from '../../../../../src/apps/client/utils/getAssetValues';
        ...
        if (order.isClosed) {
                ...
                .then((prevOrder) => {
                    getUserById(order.userId)
                        .then(async (user) => {
                            ...
                            const prevProfit = getProfit(
                                prevOrder.amount,
                                prevOrder.openingPrice,
                                prevOrder.closedPrice,
                                prevOrder.type,
                                prevAsset
                            );
                            ...
                            Promise.all([editOrderQuery(order), editUserQuery(updatedUser)])
                                .then(([order]) => {
            ...
            if (orders.length) {
                ...
                const realSymbolPrice =
                    order.type === 'buy'
                        ? calculateBuyPrice(symbolInfo.name, symbolPrice)
                        : symbolPrice;
            ...
            if (freeBalance && order.pledge > freeBalance && order.pledge > totalPledge) {
                return res
                    ...
                    .send({error: 'Залог больше свободных средств'});
            }
