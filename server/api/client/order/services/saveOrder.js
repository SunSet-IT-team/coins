...
export default function saveOrder(req, res) {
  try {
    const { assetName, amount, type, takeProfit, stopLoss, autoClose, profit } =
      req.body;
    const openingPrice = pricesController.prices[assetName];
    const openingPriceReal =
      type === "sell"
        ? openingPrice
        : calculateBuyingPrice(assetName, openingPrice);
    ...
    return saveOrderQuery(orderObj)
      .then(async () => {
        ...
        return res
          ...
          .send({ openOrders, /* closedOrders, */ countClosedOrders });
      })
      .catch(() => {
