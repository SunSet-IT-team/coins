...
class MainPage extends Component {
    ...
    filterChatsBySearch = (chats, search = this.state.search) => {
        ...
        return filteredByManager.filter((messagesByUser) => {
            ...
            return (
                user && `${user.name} ${user.surname}`.toLowerCase().includes(search.toLowerCase())
            );
        });
    };
    ...
    handleSearchChange = (event) => {
        ...
        const filteredMessages = this.state.messagesByUsers.filter((messagesByUser) => {
            ...
            return (
                user &&
                `${user.name} ${user.surname}`.toLowerCase().includes(newValue.toLowerCase())
            );
        });
    ...
    getData = () => {
        Promise.all([this.props.getMessageHistory(), this.props.getUsers()]).then(
            ([messages, users]) => {
                const allUsers = users.payload;

                const filteredUsers =
                    this.props.currentAdmin && this.props.currentAdmin.id === 'manager_id'
                ...
                const newMessagesByUsers = filteredUsers
                    .reduce((result, user) => {
                        ...
                        const sortedByDateMessages = currentUserMessages.reduce(
                            (acc, item) => {
                                if (acc[acc.length - 1][0]) {
                                    let currentItemDate = format(item.createdAt, 'P');
                                    let currentArrayDate = format(
                                        acc[acc.length - 1][0].createdAt,
                                        'P'
                                    );
                    ...
                    .sort((prev, next) => {
                        ...
                        return last(prev.messages[0]).createdAt - last(next.messages[0]).createdAt <
                            0
                            ? 1
                            : -1;
                    });
    ...
    handleMessage = (message) => {
        ...
        const index = findIndex(
            (messagesByUser) => messagesByUser.userId === userId,
            messagesByUsers
        );
        ...
        if (index === -1) {
            ...
            messagesByUsers[index].messages[messagesByUsers[index].messages.length - 1].push(
                message
            );
            if (last(last(messagesByUsers[index].messages)).receiverId === 'admin') {
                messagesByUsers[index].unvisitedMessagesCounter =
                    messagesByUsers[index].unvisitedMessagesCounter + 1;
            }
    ...
    handleTabClick = (index, userId) => () => {
        ...
        const messagesByUsersIndex = findIndex(
            (messagesByUser) => messagesByUser.userId === userId,
            messagesByUsers
        );
        messagesByUsers[messagesByUsersIndex] = {
            ...messagesByUsers[messagesByUsersIndex],
            unvisitedMessagesCounter: 0,
        };
        const showedMessagesByUsersIndex = findIndex(
            (messagesByUser) => messagesByUser.userId === userId,
            showedMessagesByUsers
        );
        showedMessagesByUsers[showedMessagesByUsersIndex] = {
            ...showedMessagesByUsers[showedMessagesByUsersIndex],
            unvisitedMessagesCounter: 0,
        };

        this.setState({activeTabId: userId, messagesByUsers, showedMessagesByUsers});
        this.props.editMessage({id: userId}).then(this.props.getUnvisitedMessageHistory);
    };
    ...
    render() {
        const {value, messagesByUsers, showedMessagesByUsers, activeTabId, usersMap, search} =
            this.state;
        ...
        return (
            <div className={styles.content} style={{position: 'relative'}}>
                ...
                <form
                    ...
                    onSubmit={messagesByUsers.length ? this.handleSubmit(activeTabId) : undefined}
                >
                    <div className={styles.contacts}>
                        {showedMessagesByUsers.map((messagesByUser, i) => {
                            const user = usersMap[messagesByUser.userId];
                            const lastMessage = messagesByUser.messages
                                ? last(messagesByUser.messages[messagesByUser.messages.length - 1])
                                : undefined;

                            return (
                                <div
                                    className={classNames(styles.contact, {
                                        [styles.active]: activeTabId === messagesByUser.userId,
                                    })}
                                    key={i}
                                    onClick={this.handleTabClick(i, user.id)}
                                >
                    ...
                    <div className={styles.messages}>
                        <div ref={this.messagesWrapper} className={styles.messagesWrapper}>
                            {(
                                messagesByUsers.find(
                                    (messagesByUser) => messagesByUser.userId === activeTabId
                                ) || {messages: []}
                            ).messages.map((item, i) => {
                                if (item.length) {
                                    return (
