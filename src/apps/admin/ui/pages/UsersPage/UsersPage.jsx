...
const SortableWrapper = ({ users, ...rest }) => (
  <List>
    {users.map((value, i) => {
      const name = `${pathOr(["name"], "", value)} ${pathOr(
        ["surname"],
        "",
        value
      )}`
      ...
      return (
        <ItemSortable key={i} name={name} value={value} index={i} {...rest} />
      )
    })}
  </List>
)
...
const tableOrderCells = [
  ...
  {
    prop: (orders) =>
      find((type) => type.type === orders.type, ORDERS_TYPE_TEXT).text,
  },
  ...
  { prop: (orders) => format(new Date(orders.createdAt), "dd.MM.yyyy HH:mm") },
  {
    prop: (orders) =>
      orders.closedAt
        ? format(new Date(orders.closedAt), "dd.MM.yyyy HH:mm")
        : "-",
  },
...
const mapDispatchToProps = (dispatch) => ({
  ...
  assignUser: (managerEmail, userEmail) =>
    dispatch(assignUser(managerEmail, userEmail)),
...
class UsersPage extends Component {
  ...
  componentDidUpdate(prevProps) {
    if (prevProps.users !== this.props.users) {
      const filteredUsers = this.filterUsersByManager(this.props.users)
      const currentActiveUser = filteredUsers.find(
  ...
  getUserTransactions = (activeUser = this.state.activeUser) => {
    ...
    if (currentAdmin && currentAdmin.id === "manager_id") {
      return transactions.filter((transaction) => {
        const transactionUser = this.props.users.find(
    ...
    return transactions.filter(
      (transaction) => transaction.userId === activeUser.id
    )
  }
      ...
      this.setState(
        {
          activeUser: clone(
            this.props.users.find(
              (user) => user.id === this.state.activeUser.id
            )
          ),
        },
        resolve
  ...
  handleWarningAgree = () => {
    ...
    this.props.deleteUsers(valueForDelete.id).then(() => {
      this.setState({
        users: this.props.users,
        activeUser:
          (activeUser === valueForDelete && users[0]) || DEFAULT_ACTIVE_SERVICE,
        valueForDelete: null,
      })
    })
  }
  ...
  render() {
    ...
    const activeUsers = (
      !inputValue ? filteredByManagerUsers : searchFilteredUsers
...
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(withStyles(materialStyles)(UsersPage))
