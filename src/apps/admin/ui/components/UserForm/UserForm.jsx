    ...
    this.initialValues = {
      ...
      phone:
        (user.country === "ua"
          ...
          : user.country === "ru"
          ? "(+7)"
          : "(+375)") + this.formatPhone(user.phone, user.country) || "",
      ...
      isSwiftDepositAvailable: user.isSwiftDepositAvailable || false,
      manager: user.manager || "",
      identity:
        user.docs && user.docs.identity
          ? [
              {
      ...
      cardBack:
        user.docs && user.docs.cardBack
          ...
          : [],
      others:
        user.docs && user.docs.others && user.docs.others.length !== 0
          ? user.docs.others.map((item) => ({
              type: item.type,
              path: item.path,
              id: uniqid(),
            }))
  ...
  handleSubmit = (values) => {
    ...
    editUser(userPayload)
      ...
      .catch(() => {
        this.setState({
          errorText:
            "Что-то пошло не так. Перезагрузите страницы и попробуйте снова",
        });
      });
  };
...
export default connect(
  null,
  mapDispatchToProps
)(withStyles(materialStyles)(UserForm));
