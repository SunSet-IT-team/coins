...
const Size = SortableElement(
    ({rowIndex, size, validationMessage, handleSizeDelete, handleSizeChange, classes}) => (
...
const Sizes = SortableContainer(({sizes, classes, ...rest}) => (
    <div>
        {sizes.map((size, i) => (
            <Size key={i} index={i} rowIndex={i} size={size} {...rest} classes={classes} />
        ))}
    </div>
));
...
class FormFieldSizes extends Component {
    ...
    handleSizeAdd = () => {
        ...
        this.props.onChange([...value, {size: '', price: '', id: uniqid()}]);
    };
