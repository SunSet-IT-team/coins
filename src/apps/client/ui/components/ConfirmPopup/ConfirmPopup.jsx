...
class ConfirmPopup extends Component {
  ...
  setOrderInfo = () => {
    ...
    const openingPriceSell =
      assetPriceWebsocketController.prices[chartSymbol.name]
    // console.log('openingPriceSell', openingPriceSell);
    const openingPriceBuy = calculateBuyingPrice(
      chartSymbol.name,
      openingPriceSell
    )
    const asset = CHART_SYMBOL_INFO_MAP[chartSymbol.name]
    // console.log('openingPriceBuy', openingPriceBuy);
    const openingSlotPrice =
      type === "sell"
        ? getOpeningSlotPrice(asset, openingPriceSell)
        : getOpeningSlotPrice(asset, openingPriceBuy)
    // console.log('openingSlotPrice', openingSlotPrice);
    const pledge = +numeral(getPledge(amountNumber, openingSlotPrice)).format(
      "0.00"
    )
  ...
  handlerSubmit = () => {
    ...
    if (
      orderInfo.amount <= 0 ||
      assetPriceWebsocketController.freeBalance <
        orderInfo.pledge + orderInfo.commission ||
        ...
        >
          <div
            className={classNames(styles.popupContainer, {
              [styles.isPopupVisible]:
                !isSuccessSend && !isFailedSend && isOpen,
            })}
          >
            <div className={styles.title}>
              {text.confirm}{" "}
              {type === "buy" ? text.confirmPurchase : text.confirmSale}
              {type === "buy" ? (
                <img
                  src="/src/apps/client/ui/components/Header/images/posArrow.svg"
                  alt=""
                />
              ) : (
                <img
                  src="/src/apps/client/ui/components/Header/images/negArrow.svg"
                  alt=""
                />
              )}
            </div>
                  ...
                  {assetGroup.imgAlone ? (
                    ...
                    <div className={styles.assetItemPair}>
                      <img
                        ...
                        src={assetGroup.imgTop}
                        alt="assets"
                      />
                      <img
                        className={styles.imgLower}
                        src={assetGroup.imgBottom}
                        alt="assets"
                      />
                    </div>
                  )}
            ...
            <div className={styles.popupItem}>
              ...
              <div className={classNames(styles.itemValue, styles.price)}>
                $ {formatNumberToString(orderInfo.pledge)}
              </div>
            </div>
            <div className={styles.popupItem}>
            ...
            <div className={styles.popupItem}>
              ...
              <div
                className={classNames(
                  buyAndSellStyles.counterContainer,
                  styles.counterContainerLeft,
              ...
              >
                <NumberFormat
                  ...
                  isAllowed={({ floatValue }) => {
                    const maxValue =
                      type === "sell"
                        ? orderInfo.openingPriceSell
                        : orderInfo.openingPriceBuy
            ...
            <div className={styles.buttonContainers}>
              {!loader.isLoading ? (
                [
                  ...
                  >
                    <img
                      src="/src/apps/client/ui/components/Header/images/removeButton.svg"
                      alt="close"
                    />
                  </button>,
                ]
          ...
          >
            <div className={styles.title}>
              {type === "buy" ? text.purchase : text.sale}
              {text.successTitle}
            </div>
            <img
              src="/src/apps/client/ui/components/ConfirmPopup/img/successIcon.png"
              alt="success"
            />
          </div>
        </div>
