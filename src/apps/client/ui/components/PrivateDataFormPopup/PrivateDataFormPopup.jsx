...
class PrivateDataFormPopup extends Component {
    ...
    componentDidUpdate(prevProps, prevState) {
        ...
        if (
            (prevProps.user !== this.props.user && this.props.user) ||
            (prevProps.isVisible !== this.props.isVisible && this.props.isVisible)
        ) {
            ...
            this.setState({
                ...
                activeCountry:
                    find((country) => country.name === this.props.user.country, COUNTRY_INFO) ||
                    this.props.user.country,
            });
        }

        if (
            this.state.isGenderOpen !== prevState.isGenderOpen &&
            !this.state.isGenderOpen &&
            !outsideClickEnabled
        ) {
        ...
        this.handleIsMobileVersion();
        this.phoneContainerRef.current.firstChild.firstChild.style.paddingLeft =
            this.callingCodeRef.current.offsetWidth + 15 + 'px';
    }
    ...
    defaultState() {
        ...
        return {
            ...
            country: {
                value: (user && user.country) || COUNTRY_INFO[0].name,
                focus: false,
                isValid: true,
            },
    ...
    handleCheckErrors = (names = []) => {
        ...
        names.forEach((name) => {
            ...
            if (isObject(property) && !isArray(property)) {
                ...
                if (name === 'phone' && value.includes(this.state.activeCountry.callingCode)) {
                    value = value.replace(this.state.activeCountry.callingCode, '');
                }

                let isValid =
                    (name === 'phone'
                        ? phone(value, {text: false})
                        : name === 'email'
                          ? email(value, {text: false})
                ...
                if (
                    name === 'newPassword' &&
                    !!this.state['newPassword'].value.length &&
                    this.state[name].value.length < 8
                ) {
                    isValid = false;
                }
                ...
                if (
                    ...
                    this.state.confirmPassword.value !== this.state.newPassword.value
                ) {
                    isValid = false;
                }

                if (
                    name === 'dateInputMaskValue' &&
                    this.props.mediaWidth < MOBILE_VERRSION &&
                    this.state.dateInputMaskValue.value.includes('_')
                ) {
                    isValid = false;
                }
    ...
    handleSubmit = (e) => {
        ...
        const thisState = this.handleCheckErrors(
            Object.keys({
                name,
                surname,
                email,
    ...
    handlerSetGender = (gender) => () => {
        this.setState({activeGender: gender, gender: {isValid: true, value: gender}});
    };

    handleSetCountry = (country) => {
        this.setState({
            activeCountry: country,
            country: {...this.state.country, value: country.name},
        });
        this.phoneContainerRef.current.firstChild.firstChild.style.paddingLeft =
            this.callingCodeRef.current.offsetWidth + 13 + 4 + 'px';
    };
    ...
    render() {
        ...
        return (
            <div className={styles.privateDataContainer} ref={this.form}>
                <div className={styles.mainDataContainer}>
                    <div className={styles.avatarContainer}>
                        <img
                            src="/src/apps/client/ui/components/PrivateDataFormPopup/images/avatar.png"
                            alt="avatar"
                        />
                    ...
                    <div className={styles.rightContainer}>
                        ...
                        {user && user.accountStatus && (
                            <div
                                className={classNames(styles.personStatus, {
                                    [styles.goldStatus]: user && user.accountStatus === 'gold',
                                    [styles.platinumStatus]:
                                        user && user.accountStatus === 'platinum',
                                    [styles.diamondStatus]:
                                        user && user.accountStatus === 'diamond',
                ...
                >
                    <div className={styles.genderContainerField} onClick={this.handleGenderClick}>
                        <div className={styles.genderFormFieldContainer}>
                            <FormInput
                                ...
                                texts={{
                                    gender: `${text.inputs.gender.placeholder}`,
                                    genderValidator: `${text.inputs.gender.validator}`,
                                }}
                                name="gender"
                        ...
                        <div className={styles.genderSelect}>
                            ...
                            <div
                                className={classNames(styles.innerGenderContainer, {
                                    [styles.innerGenderContainerOpen]: isGenderOpen,
                                })}
                            >
                                {GENDER.filter((item) => item.name !== activeGender).map((item) => (
                                    <div
                                        key={item.id}
                                        className={styles.genderContainer}
                                        onClick={this.handlerSetGender(item.name)}
                                    >
                    ...
                    <div className={styles.birthDateContainerField}>
                        <div className={styles.birthDateContainerFormInput}>
                            <FormInput
                                isError={
                                    ...
                                    !this.state['dateInputMaskValue'].isValid
                                }
                                texts={{
                                    date: `${text.inputs.date.placeholder}`,
                                    dateValidator: `${text.inputs.date.validator}`,
                                }}
                                name="date"
                                onFocus={this.onFocus}
                                onBlur={this.onBlur}
                        ...
                        <img
                            src="/src/apps/client/ui/components/PrivateDataFormPopup/images/date.svg"
                            alt=""
                        />
                    </div>
                    <div
                        ref={this.phoneContainerRef}
                        className={classNames(styles.phoneContainerField, {
                            [styles.phoneInputActive]: this.state.phone.focus,
                        })}
                    >
                        {/* <InputMask */}
                        {/*    mask={activeCountry.callingCode} */}
                        {/*    maskChar=" " */}
                        ...
                        <FormInput
                            ...
                            texts={{
                                phone: `${text.inputs.phone.placeholder}`,
                                phoneValidator: `${text.inputs.phone.validator}`,
                            }}
                            name="phone"
                            onFocus={this.onFocus}
                            onBlur={this.onBlur}
                            ...
                            focus={this.state.phone.focus}
                        />
                        <div ref={this.callingCodeRef} className={styles.callingCode}>
                            {activeCountry.callingCode}
                        </div>
                    </div>
                    ...
                    <div className={styles.cityContainerField}>
                        <FormInput
                            ...
                            texts={{
                                city: `${text.inputs.city.placeholder}`,
                                cityValidator: `${text.inputs.city.validator}`,
                            }}
                            name="city"
                            onFocus={this.onFocus}
                            onBlur={this.onBlur}
                    ...
                    <div className={styles.addressContainerField}>
                        <FormInput
                            ...
                            texts={{
                                address: `${text.inputs.address.placeholder}`,
                                addressValidator: `${text.inputs.address.validator}`,
                            }}
                            name="address"
                            onFocus={this.onFocus}
                            onBlur={this.onBlur}
                    ...
                    <div className={styles.accountNumberContainerField}>
                        <FormInput
                            ...
                            texts={{
                                accountNumber: `${text.inputs.accountNumber.placeholder}`,
                                accountNumberValidator: `${text.inputs.accountNumber.validator}`,
                            }}
                            name="accountNumber"
                            onFocus={this.onFocus}
                            onBlur={this.onBlur}
                    ...
                    <div className={styles.emailContainerField}>
                        <FormInput
                            ...
                            texts={{
                                email: `${text.inputs.email.placeholder}`,
                                emailValidator: `${text.inputs.email.validator}`,
                            }}
                            name="email"
                            onFocus={this.onFocus}
                            onBlur={this.onBlur}
                    ...
                    <div className={styles.newPasswordContainerField}>
                        <FormInput
                            ...
                            texts={{
                                newPassword: `${text.inputs.newPassword.placeholder}`,
                        ...
                        />
                        <div
                            className={classNames(styles.bottomLabel, styles.bottomPasswordLabel, {
                    ...
                    <div className={styles.clearPasswords} onClick={this.handleClearPassword}>
                        {text.clear}
                    </div>
                ...
                <div
                    className={classNames(styles.successPopup, {
                        [styles.active]: isSuccesPopupShow,
                    })}
                >
                    <p className={styles.successPopupTitle}>Сохранение успешно</p>
                    <img
                        src="/src/apps/client/ui/components/PrivateDataFormPopup/images/successIcon.png"
                        alt=""
                    />
                </div>
            </div>
