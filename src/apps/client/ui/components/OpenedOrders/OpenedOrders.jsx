...
class OpenedOrders extends Component {
    ...
    handleAutoCloseOrder = (payload) =>
        this.props.autoCloseOrder({
            ...payload,
            closedOrdersList: this.state.closedOrders,
        });
    ...
    handleConfirmCloseDeal = (order) => () => {
        this.setState({
            isLoading: {...this.state.isLoading, loading: true, id: order.id},
        });
        this.props
            .closeOrder({id: order.id, closedOrdersList: this.state.closedOrders})
            .then(() => {
                this.setState({
                    isLoading: {...this.state.isLoading, loading: false},
                    closeOrderIndex: null,
                });
            });
    };
    ...
    getClosedOrders = (orders) => {
        return orders.map((order) => {
            ...
            const profit =
                order.profit ||
                getProfit(order.amount, order.openingPrice, order.closedPrice, order.type, asset);
    ...
    render() {
        ...
        const {openOrders, closeOrderIndex, isLoading} = this.state;

        return (
            <div className={styles.openPositionInnerContaimer}>
