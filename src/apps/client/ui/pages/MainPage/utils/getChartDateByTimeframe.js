...
export default function (time, timeframe) {
    ...
    switch (timeframe) {
        ...
        case '5':
            ...
            return (
                +set(new Date(date), {minutes: full5Minutes, seconds: 0, milliseconds: 0}) / 1000
            );
        case '15':
            const full15Minutes = Math.floor(minutes / 15) * 15;

            return (
                +set(new Date(date), {minutes: full15Minutes, seconds: 0, milliseconds: 0}) / 1000
            );
        case '30':
            const full30Minutes = Math.floor(minutes / 30) * 30;

            return (
                +set(new Date(date), {minutes: full30Minutes, seconds: 0, milliseconds: 0}) / 1000
            );
        case '60':
        ...
        case 'W':
            ...
            return (
                +set(subDays(new Date(time), neededDayOfWeek), {
                    hours: 3,
                    minutes: 0,
                    seconds: 0,
                    milliseconds: 0,
        ...
        case 'M':
            return (
                +set(new Date(time), {date: 1, hours: 3, minutes: 0, seconds: 0, milliseconds: 0}) /
                1000
            );
    }
}
